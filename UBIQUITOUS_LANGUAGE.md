...
| **Mount** | Exposes a tar archive, squashfs image, or erofs image in the workspace for the worker, read-only or as a writable tmpfs overlay | Attach, bind |
...
| **Reservation** | The resource-version-guarded metadata update that wins a warm pod and stamps its final workload identity before activation | Lock, lease |
| **Claim** | The complete take of one warm pod: Reservation followed by token-authenticated Activation | Checkout |
| **Claim token** | The per-pod credential (HMAC of the pod name under the install key) that authorizes a claim | Secret, password |
| **Inventory** | What a Pool backend supplies to the shared claim module: how to list, create, reserve, address, and discard warm units | Backend, provider |
