...
- `cmd/jobs-service` — main orchestration service (HTTP API on :8080, metrics pushed over OTLP)
...
- `cmd/workload-sidecar` — reverse proxy in front of every serving workload: deployment replicas (created or pool-claimed) and sandboxes (readiness, drain, per-request timeout, request counting, and the claim endpoint)
