...
func waitEvent(t *testing.T, q *captureQueue) ResponseData {
	t.Helper()
	select {
	case <-q.ch:
	...
	return q.last().Payload.Data.(ResponseData)
}
...
func TestBrokerAsyncBoundsRequestPath(t *testing.T) {
	...
	data := waitEvent(t, queue)
	if got := data.RequestPath; len(got) != maxEchoedPathBytes {
...
func TestBrokerAsyncCallbackBase64ForBinaryBody(t *testing.T) {
	...
	decoded, err := base64.StdEncoding.DecodeString(data.Body)
...
func TestBrokerAsyncReportsHoldTimeout(t *testing.T) {
	...
	data := waitEvent(t, queue)
	if data.Error == nil || data.Error.Code != "deployment_no_capacity" {
...
func (f *fakeRecorder) RecordActivatorHold(_ context.Context, _, outcome string, _ float64) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.holds = append(f.holds, outcome)
}

func (f *fakeRecorder) RecordActivatorQueueDelta(_ context.Context, _ string, delta int64) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.queue += delta
}

func (f *fakeRecorder) RecordActivatorRaise(context.Context, string) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.raises++
}

func (f *fakeRecorder) RecordActivatorAsync(_ context.Context, _, result string) {
	f.mu.Lock()
	defer f.mu.Unlock()
