...
// The claim's post-phase artifacts run when the workload is told to stop — the
// snapshot-on-shutdown path. Ordering is the whole point: after the drain, so
// nothing is still writing, and before the release, so an artifact that archives
// a mount still has one to read.
func TestPoolClaim_RunsPostPhaseArtifactsOnShutdown(t *testing.T) {
	cfg := poolConfig(t)
	cfg.Mounts = true
	cfg.MaxDrain = 5 * time.Second
	mounter := &fakeMounter{}
	...
	// A mount to read from, and a post-phase artifact that snapshots it.
	if status := postActivate(t, p, testClaimToken, workload.ClaimRequest{
		ActivationID: "act-snap",
		Command:      "serve",
		Port:         backendOnLoopback(t, "ok"),
		Artifacts: []artifact.Artifact{
			&artifact.Mount{ID: "tree", In: "data.sqfs", Out: "work"},
			&artifact.Write{ID: "snap", In: "snapshot", Out: "out/snap.txt", Depends: artifact.WorkloadDependency},
		},
	}); status != http.StatusOK {
		t.Fatalf("claim: got %d (%+v)", status, claimState(t, p))
	}
	<-execCh

	// Nothing post-phase has run yet: the workload is still going.
	if _, err := os.Stat(filepath.Join(cfg.Workspace, "out/snap.txt")); err == nil {
		t.Fatal("a post-phase artifact must not run while the workload is serving")
	}
	...
	// It ran on the way out, and the mount outlived it.
	if _, err := os.Stat(filepath.Join(cfg.Workspace, "out/snap.txt")); err != nil {
		t.Errorf("the post-phase artifact should have run on shutdown: %v", err)
	}
	if mounted, released := mounter.counts(); released != mounted {
		t.Errorf("mounts must be released after finalizing: %d mounted, %d released", mounted, released)
	}
}

// A claim with nothing post-phase has nothing to do on the way out.
func TestPoolClaim_NoPostPhaseNothingToFinalize(t *testing.T) {
	cfg := poolConfig(t)
	execCh := shimReader(t, cfg.Workspace)
	ctx, cancel := context.WithCancel(t.Context())
	p := New(cfg)
	...
	if status := postActivate(t, p, testClaimToken, workload.ClaimRequest{
		ActivationID: "act-plain",
		Command:      "serve",
		Port:         backendOnLoopback(t, "ok"),
		Artifacts:    []artifact.Artifact{&artifact.Write{ID: "pre", In: "x", Out: "pre.txt"}},
	}); status != http.StatusOK {
		t.Fatalf("claim: got %d", status)
	}
	<-execCh
