...
func (a *Archive) Apply(ctx context.Context, basePath string) *Result {
	...
	case "erofs":
		...
		return &Result{Status: "success"}
	default:
		return &Result{Status: "failed", Error: fmt.Errorf("%w: unsupported archive format: %s (supported: tar, squashfs, erofs)", ArchiveUnknownFormat, a.Format)}
	}
}
...
func (a *Archive) applyTar(srcPath, destPath string) *Result {
	...
	case "lz4hc":
		...
		w = lz4Writer
	default:
		return &Result{Status: "failed", Error: fmt.Errorf("%w: unsupported tar compression: %q (supported: gzip, zstd, lz4, lz4hc, none)", ArchiveCompressionUnsupported, a.Compression)}
	}
