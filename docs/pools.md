...
Pool responsibilities are deliberately split. Consumer services perform request-path reservation, activation, and readiness and own live claimed-workload lifecycle. The generic `pool-controller` owns warm-pod inventory for both Revision and sandbox pools: it creates replacements and removes poisoned, abandoned reservations, and obsolete or shape-stale unclaimed pods. The chart runs the same binary independently for each enabled pool kind, preserving each plane's placement and S3 credentials. It keeps running with an empty pool list so removing the final pool also cleans up its standing capacity; accepted live claims are never reclaimed by inventory cleanup.
