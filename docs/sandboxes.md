...
[Pool activations](pools.md#artifacts) follow the same rules over the same machinery. Deployment revisions do not support mounts, but the reason is that nothing wires it rather than anything structural: a revision materializes its artifacts in a separate `artifact-pre` init container that then **exits**, taking any mount it made with it, and the resident sidecar that outlives it is never told what those artifacts were. The pod shape would allow it — that sidecar is a native sidecar, starting after the artifacts and before the worker, exactly like a warm pod's — so supporting it is work, not a redesign. The cost would be a privileged container in every replica of every mounting revision.
